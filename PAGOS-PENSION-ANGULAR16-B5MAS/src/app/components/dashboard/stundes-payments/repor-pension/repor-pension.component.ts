import { Component, ElementRef, ViewChild, ViewContainerRef, TemplateRef, OnInit } from '@angular/core';
...
export class ReporPensionComponent implements OnInit{
  ...
  constructor(private _adminService:AdminService ,private _route: ActivatedRoute,private _router: Router,private el: ElementRef, private viewContainerRef: ViewContainerRef,private _configService:ConfigService) {
      ...
      if(responese.data){
        this.info=responese.data;
          ...
          if(this._configService.getDirector()==''||this._configService.getDelegado()==''){
            this._adminService.listar_admin(this.config_sistem.token).subscribe((response) => {
  ...
  llamarRetirado(){
    ...
    localStorage.getItem('penest_0')&&localStorage.getItem('estudiantes_0')){
      var j=0;
      do {
        ...
        if(!localStorage.getItem('penest_'+j)&&!localStorage.getItem('estudiantes_'+j)){
            j=-1;
        }else{
          j++;
        }              
  ...
  reporteRetirado(){
    ...
    this.penest.forEach((element:any) => {
      if(element.idestudiante.estado == 'Desactivado' &&	new Date(element.anio_lectivo).getTime()==new Date(element.idestudiante.anio_desac).getTime()
      ){
  ...
  reporteBeca(){
    ...
    this.pensionesestudiantearmado = this.pensionesestudiantearmado.sort(function (a: any, b: any) {
      if (parseInt(a.curso)  > parseInt(b.curso)) {
        return 1;
      } else if (parseInt(a.curso) < parseInt(b.curso)) {
        return -1;
...
cambiarSeleccionTodo(val: any) {
  ...
  imprimir(){
    ...
    if(this.tipreport=='Pension'||this.tipreport=='Rubro'||this.tipreport=='PensionRubro'){
      this.cursos2.forEach((element:any) => {
        this.paralelo.forEach((element1:any) => {
          ...
          if(element.seleccionado==false||element1.seleccionado==false){
            let off=document.getElementById((element.name+element1.name).toString());
            if(off){
              off.style.display = 'none';
            }
          }
    
          if((this.cursos2[0].seleccionado==true||element.seleccionado==true)&&(element1.seleccionado==true||this.paralelo[0].seleccionado==true)){
            let off=document.getElementById((element.name+element1.name).toString());
            if(off){
              off.style.display = '';
            }
  ...
  llamarimprimir(){
    ...
    if (popupWin&&table) { // Comprueba si la ventana emergente se abrió correctamente
