...
detalle_data(val: any) {
  ...
  if (this.active != val) {
    ...
    if(this.actualizar_dashest==false&&this.active==0){
      ...
      if(localStorage.getItem('dia')&&JSON.stringify(this.pdffecha)==localStorage.getItem('config')){
...
armado_matriz(val:any,costosextrapagos:any,costopension:any,costomatricula:any){
  ...
  if(this.actualizar_dashest==true){
    this._adminService.obtener_detallespagos_admin(this.token, this.config[val].anio_lectivo).subscribe((response) => {
            ...
            if(response.data){
              ...
              if (this.penest != undefined) {
                ...
                this.cursos.forEach((element:any) => {
                    this.cursos2.push({name:element,seleccionado:false});
              ...
              for(var i=0;i<this.deteconomico[0].data.length;i++){
                datatotal.push(this.deteconomico[1].data[i]+this.deteconomico[2].data[i]);
              }
              this.deteconomico.push({
                label:'Total',
                data:datatotal,
