...
export class IndexDocumentComponent implements OnInit {
  ...
  obtener(id: any) {
    if (this.fact.find((element: any) => element.id == id) == undefined) {
      this._adminService
        .obtener_documento_admin(id, this.token)
        .subscribe((response) => {
          if (response.fact) {
  ...
  editar(element: any) {
    ...
    if (element.valor > 0) {
      this._adminService
        .actualizar_documento_admin(element, element._id, this.token)
        .subscribe((response) => {
  ...
  eliminar_todo() {
    ...
    this.documentos.forEach((element: any) => {
      if (element.ckechk == 1) {
        this._adminService
          .eliminar_documento_admin(element.element._id, this.token)
          .subscribe(
            (response) => {
              con++;
              if (con == ultimo) {
                iziToast.success({
                  ...
                  position: 'topRight',
                  message:
                    'Se eliminó correctamente el(los) documento.' +
                    '(' +
                    con +
                    ')',
                });
  ...
  filtrar_documento() {
    ...
    if (this.filtro) {
      ...
      this.documentos = this.documentos.filter(
        (item) =>
          term.test(item.element.documento) ||
