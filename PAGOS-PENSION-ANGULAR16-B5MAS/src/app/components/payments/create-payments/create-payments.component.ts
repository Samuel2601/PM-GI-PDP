...
export class CreatePaymentsComponent implements OnInit {
  ...
  btnbuscardocumento = document.getElementById('btnbuscardocumento') as
    | HTMLButtonElement
    | undefined;
  btnnuevodocumento = document.getElementById('btnnuevodocumento') as
    | HTMLButtonElement
    | undefined;
  btnAgregar = document.getElementById('btnAgregar') as
    | HTMLButtonElement
    | undefined;
      ...
      .subscribe(
        (response) => {
          if (response.data) {
            ...
            $('#btnbuscardocumento, #btnnuevodocumento').prop(
              'disabled',
              false
            );
          } else {
  ...
  isPensionValid(i: number): boolean {
    const isValid =
      ...
      (this.pension[i].extrapagos === undefined &&
        this.pension[i].idanio_lectivo.extrapagos !== undefined) ||
  ...
  actualizar_valor() {
    this._adminService
      ...
      .subscribe((response) => {
        ...
        finAnioLectivo.setMonth(inicioAnioLectivo.getMonth() + 10); // Año lectivo dura 10 meses

        // Revisamos si la fecha actual está dentro del rango del año lectivo
        this.checkfecha =
          fechaActual >= inicioAnioLectivo && fechaActual <= finAnioLectivo;
  ...
  addDocumento2(debe: any) {
    ...
    if (this.valor >= 0.001) {
      ...
      if (
        parseFloat(
          (
            parseFloat(this.documento_select.valor) - this.valorigualdocumento
          ).toFixed(2)
        ) > 0
      ) {
        this._adminService
          ...
          .subscribe((response) => {
            ...
            if (this.checkfecha) {
              if (this.tipo === 0) {
                // Caso de matrícula
                if (this.num_pagos > 1) {
                  est = 'Pago atrasado'; // Si ya ha pasado más de un mes, la matrícula está atrasada
                } else {
                  est = 'Pago a tiempo'; // Si es dentro del primer mes, la matrícula está a tiempo
                }
              } else if (this.tipo >= 1 && this.tipo <= 10) {
                // Caso de pensiones (1-10)
                if (this.tipo < this.num_pagos) {
                  est = 'Pago atrasado'; // Si la pensión no ha sido pagada en el mes correspondiente
                } else if (this.tipo === this.num_pagos) {
                  est = 'Pago a tiempo'; // Pensión pagada a tiempo
                } else {
                  est = 'Pago anticipado'; // Pensión pagada por adelantado
                }
              } else {
                // Si no es matrícula ni pensión (otros valores), no debe tener estado
                est = ''; // No se asigna estado a rubros o valores adicionales
              }
            } else {
              est = 'Pago atrasado'; // Si no está dentro del año lectivo, está atrasado
            }
            ...
            if (
              parseFloat(
                (
                  parseFloat(this.documento_select.valor) -
        ...
        this.dpago.forEach((element: any) => {
          if (this.documento_select.documento == element.titulo_documento) {
            this.valorigualdocumento =
              parseFloat(element.valor) + this.valorigualdocumento;
          }
        });
      }
    } else {
  ...
  quitar(id: any, valor: any, tipo: any) {
    ...
    if (tipo == 0) {
      ...
      if (tipo > 0 && tipo <= 10) {
        ...
        var aux = this.arr_rubro_const.find(
