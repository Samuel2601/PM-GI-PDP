...
export class DetailStudentsComponent implements OnInit {
  ...
  detalle_data(val: any) {
    ...
    if (
      this.estudiante.f_desac != undefined &&
      this.estudiante.estado == 'Desactivado'
    ) {
      let mes =
        (new Date(this.estudiante.f_desac).getFullYear() -
          new Date(
            this.pensionesestudiante[this.idexpension].anio_lectivo
          ).getFullYear()) *
        12;
      mes -= new Date(
        this.pensionesestudiante[this.idexpension].anio_lectivo
    ...
    this._adminService
      .obtener_detalles_ordenes_estudiante_abono(
        this.pensionesestudiante[this.idexpension]._id,
        this.token
      )
      .subscribe((response) => {
        console.log('arr_pagos', response);
        ...
        this._adminService
          .obtener_detalles_por_estudiante(this.id, this.token)
          .subscribe((response) => {
            console.log(response);
            if (response.data != undefined) {
  ...
  armado(tiempo: any) {
    this.progressBarService.setProgress(
      this.progressBarService.getProgress() + 20
    );
    ...
    if (this.detalles != undefined) {
      ...
      for (var j = 0; j <= tiempo; j++) {
        ...
        if (j == 0) {
          if (
            (this.pensionesestudiante[this.idexpension].matricula == 1 &&
              valor != 0) ||
          ...
          if (
            new Date(
              new Date(f).setMonth(new Date(f).getMonth() + j - 1)
            ).getMonth() == 11 &&
            j != 0
          ) {
            // ////console.log('Pension',this.config.pension);
            this.diciembre = new Date(f).setMonth(
              new Date(f).getMonth() + j - 1
            );
            ...
            if (this.auxbecares != undefined && j > 0 && j <= this.auxbecares) {
              ...
              this.total_pagar =
                this.total_pagar +
                this.pensionesestudiante[this.idexpension].val_beca;
            } else {
              if (j != 0) {
                //  ////console.log('Pension',this.config.pension);
                this.total_pagar = this.total_pagar + this.config.pension;
        ...
        if (j == 0) {
          var porpagar =
            this.pensionesestudiante[this.idexpension].idanio_lectivo
              .matricula - valor;
          ...
          var porpagar =
            this.pensionesestudiante[this.idexpension].idanio_lectivo.pension -
            valor;
          var beca = 0;
          if (
            this.pensionesestudiante[this.idexpension].condicion_beca == 'Si'
          ) {
            this.arr_becas.forEach((element: any) => {
              if (
                new Date(
                  new Date(f).setMonth(new Date(f).getMonth() + j - 1)
                ).getTime() == new Date(element.titulo).getTime()
              ) {
                porpagar =
                  this.pensionesestudiante[this.idexpension].val_beca - valor;
                beca = 1;
              }
            });
          }
  ...
  imprimir() {
    ...
    const plantillaHTML =
      ...
      ` ` +
      this.pensionesestudiante[this.idexpension].curso +
      this.pensionesestudiante[this.idexpension].paralelo +
      this.pensionesestudiante[this.idexpension].especialidad +
      `(` +
      new Date(
        this.pensionesestudiante[this.idexpension].anio_lectivo
      ).getFullYear() +
