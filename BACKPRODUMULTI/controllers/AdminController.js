	...
	if (req.user.email == 'samuel.arevalo@espoch.edu.ec') {
		try {
			...
			if (error instanceof mongoose.Error.ValidationError) {
				res.status(400).send({ message: 'Error de validación. Por favor, verifica tus datos.' });
			} else {
				res.status(500).send({ message: 'Algo salió mal. Por favor, intenta nuevamente más tarde.' });
			}
		}
	} else {
...
const actualizar_config_plana = async function (req, res) {
	if (req.user) {
		try {
			...
			let conn = mongoose.connection.useDb(req.user.base);
			const ConfigPModel = conn.model('ConfigP', ConfigPSchema);
			let config = await ConfigPModel.findByIdAndUpdate(id,data,{ new: true });
...
const obtener_info_admin = async (req, res) => {
	if (req.user) {
		try {
			...
			let info = admin_arr.find(element => element.idadmin.base == req.user.base);
...
const listar_registro = async function (req, res) {
	if (req.user) {
		try {
			...
			if (hasta) {
				condicionesDeBusqueda.createdAt = { ...condicionesDeBusqueda.createdAt, $lte: new Date(hasta) };
			}
...
const actualizar_admin = async function (req, res) {
	if (req.user) {
	  try {
		...
		let duplicateEmailAdmins = await Admin.find({ email: data.email, _id: { $ne: id } });
		let duplicateDniAdmins = await Admin.find({ dni: data.dni, _id: { $ne: id } });
  
		if (duplicateEmailAdmins.length > 0 || duplicateDniAdmins.length > 0) {
		  res.status(200).send({ message: 'Correo o cédula ya existente', data: undefined });
		} else {
		  ...
		  if (data.password !== '') {
			bcrypt.hash(data.password, null, null, async function (err, hash) {
			  if (hash) {
				...
				res.status(200).send({ message: 'Error al cifrar la contraseña', data: undefined });
			  }
			});
		  } else {
...
async function actualizarInformacionAdmin(id, data, res, req, admin) {
	...
	if ((admin_arr3.length !== 0 && data.rol !== 'direc') || admin_arr3.length === 0) {
	  ...
	  await crearRegistroActualizacion(req, admin, data);
	  res.status(200).send({ message: 'Actualizado con éxito. Se cambió el rol a Colecturía' });
	}
};
...
const obtener_pagos_admin = async function (req, res) {
	if (req.user) {
		try {
			...
			// Realizar la búsqueda con las condiciones
			pagos = await Pago.find(condicionesDeBusqueda).populate('estudiante').sort({ createdAt: -1 });
...
const obtener_pagos_dash = async function (req, res) {
	if (req.user) {
		try {
			...
			// Realizar la búsqueda con las condiciones
			pagos = await Dpago.find(condicionesDeBusqueda, { valor: 1, idpension: 1, estado: 1, createdAt: 1 })
				.populate({
					path: 'idpension',
				...
				if(err){
					...
					client.GenerarFactura(data, function(err1,result){
						if(err1){
							...
							if(result.respuesta.respuestaGenerada=='Transaccion Exitosa'
		...
		try {
			...
			try {
				...
				if(valid2.length==0){
					...
					res.status(200).send({ pago: pago, message: 'no se puede registrar' });
				}else{
