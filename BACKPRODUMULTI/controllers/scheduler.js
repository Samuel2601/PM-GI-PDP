...
async function actualizarNumpensionEnTodasLasBases() {
  try {
    ...
    for (const dbName of basesDeDatos) {
      ...
      const ultimoConfig = await Config.findOne({
        anio_lectivo: { $type: "date" },
      }).sort({ anio_lectivo: -1 });
      ...
      if (ultimoConfig) {
        ...
        const diffMonths =
          diffYears * 12 + (fechaActual.getMonth() - anioLectivo.getMonth());
  ...
  try {
    ...
    for (const dbName of basesDeDatos) {
      ...
      if (ultimoConfig) {
        ...
        console.log(
          `[${dbName}] Programando tarea para ejecutar el ${proximoAnioLectivo} con la expresión cron: ${cronExpresion}`
        );

        cron.schedule(cronExpresion, () => {
          console.log(
            `[${dbName}] Ejecutando tarea programada: actualizarNumpension`
          );
        ...
        console.log(
          `[${dbName}] No se encontró ningún documento para programar la tarea.`
        );
      }
    }
  } catch (error) {
