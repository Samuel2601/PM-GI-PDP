...
//const X2JS = require('@andxor/x2js');
...
function p_generar_factura_xml(){
    ...
    return {xmlresult:xmlAsStr,clave:estructuraFactura[tipoComprobante].infoTributaria.claveAcceso};
}
...
//fs.writeFile(p_generar_factura_xml(), 'factura-'+moment().format('YYYYMMDD-hhmm')+'.xml', p_firmar_factura);
    
function p_firmar_factura(path) {
    console.log("1",path,fs.existsSync(path));
    //fs.readFileSync(path, "utf8")
    fs.open(path, 'r', function(err,factura) {
        if(err!=undefined){
            console.log(err);
        }
        console.log("2",factura );
        p_generar_xades_bes(factura, function(factura_firmada, claveAcceso){
            return {factura_firmada:factura_firmada,claveAcceso:claveAcceso}
            console.log('factura_firmada:', factura_firmada);
            console.log('base 64 dex factura firmada: ', forge.util.encode64(factura_firmada));
...
function p_generar_xades_bes(factura, callback){
    ...
    const xmlDoc = new jsdom.JSDOM(fs.readFileSync(factura), {
        //url: factura,
      ...
      //console.log(xmlDoc);
    //console.log(xmlDoc.window.document.location);
   // console.log(xmlDoc.window);
    //console.log(xmlDoc.window.document.);
   // console.log(xmlDoc.window.document.getElementsByTagName("factura"));
    //var parser = new jsdom();
    //var xmlDoc = parser.JSDOM(factura,"text/xml");
    
    var claveAcceso = xmlDoc.window.document.getElementsByTagName("claveAcceso")[0].childNodes[0].nodeValue;
    console.log(claveAcceso);
     //xmlDoc.getElementsByTagName("claveAcceso")[0].childNodes[0].nodeValue;


    var p12_path = './facturas/accesos/DAVID DANIEL PANCHI CANDONGA 020223164208.p12';
    var p12xxx = fs.readFileSync(p12_path,'base64');
    
    var pwdCert = 'DpMaMd919314';
    //console.log(p12xxx);
    generarFirma(p12xxx, factura, pwdCert, function(firma, certificado, modulus, firma_pem, certificado_pem, modulus_pem, 
        ...
        "\nissuerName\n",issuerName);
        var aux=xmlDoc.window.document.documentElement.innerHTML;
        ///console.log("414",aux);
       // xmlDoc.window.document.removeChild(aux);
        
        var sha1_factura = sha1_base64(xmlDoc.window.document.documentElement.innerHTML);
        ...
        p_firmar(p12xxx, SignedInfo_para_firma, pwdCert, function(firma_SignedInfo){
            ...
            xades_bes += '</ds:Signature>';
                ...
                var aux2 = xmlDoc.window.document.documentElement.innerHTML.toString().search("</factura>");
                //console.log(aux2);
                var inicio= xmlDoc.window.document.documentElement.innerHTML.toString().slice(0,aux2);
                var fin=xmlDoc.window.document.documentElement.innerHTML.toString().slice(aux2,xmlDoc.window.document.documentElement.innerHTML.toString().length);
                //console.log("todo",inicio+xades_bes+fin)
           // console.log("Xades",xades_bes);
            
            //FIN DE LA FIRMA DIGITAL 
            //var aux2=xmlDoc.window.document.documentElement.innerHTML;
            //var aux3=xmlDoc.window.document.getElementsByTagName("factura");
            
           // console.log("Factura",(xmlDoc.window.document.documentElement.innerHTML));
            

           callback(inicio+xades_bes+fin,claveAcceso);
    ...
    if (p12File !== undefined && infoAFirmar !== undefined) {
      ...
      //  reader.readAsArrayBuffer(p12File);
       ...
       // reader.onloadend = function () {
            arrayBuffer = Buffer.from(p12File);//reader.result;
           ...
           // console.log(p12Asn1);
            ...
            var issuerName = cert.issuer.attributes[3].shortName + '=' + cert.issuer.attributes[3].value + ', ' +
...
module.exports = {
   ...
   // bigint2base64,
    ...
    p_obtener_secuencial,
    p_calcular_digito_modulo11,
    p_comprobar_numero_cedula,
    p_generar_xades_bes
};